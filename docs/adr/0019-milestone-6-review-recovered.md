# ADR 0019 — Milestone 6 review (recovered)

> **Recovered in Milestone 11.** This review was written on
> `claude/m6-architecture-review-56mj9k` and never merged: a _different_ Milestone 6 review
> (`b753270`, "bound the render pool, fix CI's silent timeout") reached the trunk instead, and this
> one was left behind with its five fixes and its adversarial test suite. It was found by auditing
> every remote branch against the trunk during Milestone 11 and is restored here verbatim apart
> from this note and the number: it was filed as ADR 0011, which the surviving line had already
> used for the Milestone 7 observation UI. The fixes and tests it describes are now in the trunk;
> the versions it quotes (engine 0.5.0, protocol 2) are the ones current when it was written.
