...
| Evolutionary diversity / generalist collapse      | Partially — capped seeds carried 30% less variation                                        | Cap removed from the ecology; variation now mutation-selection limited                                                      | Sweep trait-sd column                                                                                                      | Median sd 0.0347; the twelve measured seeds no longer reach the cap, so its slot-order refusal never fires on them (the mechanism itself remains in `reproduction.ts` — ADR 0026 §3); leaner-world cost stated in §2d | **PASS** (with stated trade-off)                                   |
